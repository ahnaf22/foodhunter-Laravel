...
<div class="container" > 
    <div class="card">
            @if($orders->count() == 0)
            <div class="container jumbotron text-center"> <h3>No orders</h3> </div>
            @else
            <h4 class="card-header container text-center bg-danger text-white"><strong>Shop Orders of {{$user->shop->name}}</strong></h4>
                ...
                <table class="table text-center">
                    ...
                    <tr>
                        ...
                        <td >
                            ...
                            <form action="{{route('admin.shop.order.view')}}"  method="post">
                                @csrf
                                <input type="hidden" name="order_id" value="{{$order->id}}">
                                <button type="sumbit" class="btn btn-warning" >view</button>
                            </form>
                            <br>
                        ...
                        @if(!$order->is_completed && $order->is_confirmed)
                        <td>
                        <form action="{{route('admin.shop.order.complete')}}"  method="post">
                                @csrf
                                <input type="hidden" name="order_id" value="{{$order->id}}">
                                <button type="sumbit" class="btn btn-success" >Complete</button>
                            </form>
                        </td>
                        @endif
                         <!-- deletemodal -->
